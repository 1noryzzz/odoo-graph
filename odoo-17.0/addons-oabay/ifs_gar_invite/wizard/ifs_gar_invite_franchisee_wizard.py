...
class GuaranteeAccountsRecInviteFranchiseeWizard(models.TransientModel):
    _name = 'ifs.gar.invite.franchisee.wizard'
    ...
    def action_confirm(self):
        ...
        if not invite_franchisee.exists():
            invite_franchisee = self.env['ifs.gar.invite.franchisee'].create({
            ...
            return {
                ...
                'params': {
                    'type': 'success',
                    'message': _(f'已创建对合伙人{invite_franchisee.name} 的邀约，请继续完善信息并发出邀请！'),
                    'next': {
                        'name': _('合伙人邀约'),
                        'type': 'ir.actions.act_window',
                        'view_mode': 'form',
                        'views': [[False, 'form']],
                        'res_model': 'ifs.gar.invite.franchisee',
                        'res_id': invite_franchisee.id,
                        'target': 'current',
                        'context': {
                            'open_wizard': True,
                            'wizard_action': {
            ...
            return {
                ...
                'params': {
                    'type': 'info',
                    'message': _(f'合伙人{invite_franchisee.name}已被邀请！'),
                    'next': {
                        'name': _('合伙人邀约'),
                        'type': 'ir.actions.act_window',
                        'view_mode': 'form',
