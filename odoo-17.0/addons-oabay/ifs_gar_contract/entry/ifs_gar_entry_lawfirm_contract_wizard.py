...
class GuaranteeAccountsRecEntryLawfirmContractWizard(models.TransientModel):
    _name = 'ifs.gar.entry.lawfirm.contract.wizard'
    _inherit = ['ifs.gar.entry.step']
    _description = '律师事务所进件流程--合同协议'
    ...
    p10_contract_info_id = fields.Many2one(
        'ifs.contract.info', related='entry_id.p10_contract_info_id', string='保理方与律师事务所合作框架协议')
    ...
    @api.depends('f42_contract_state', 'f43_contract_state', 'p10_contract_state')
    def _compute_contract_state(self):
        for contract in self:
            if contract.f42_contract_state in ['committed', 'signed'] and contract.f43_contract_state in ['committed', 'signed'] and contract.p10_contract_state in ['committed', 'signed']:
    ...
    def sign_contract(self):
        if self.user_has_groups('ifs_gar_invite.group_ifs_gar_lawfirm_entry'):
            contract_info_ids = []
            if self.p10_contract_info_id:
                contract_info_ids.append(self.p10_contract_info_id.id)
