...
class GuaranteeAccountsRecEntryLawfirm(models.Model):
    ...
    def start_step(self):
        if self.state == 'rejected':
            first_model = 'ifs.gar.entry.lawfirm.reject.info.wizard'
            return {
    ...
    def action_approve(self):
        ...
        if self.can_confirm:
            ...
            partner_lawfirm_sudo = self.env['ifs.partner.lawfirm'].sudo()
            lawfirm = partner_lawfirm_sudo.search([
                ('ifs_company_id', '=', self.ifs_company_id.id)], limit=1)
            if not lawfirm.exists():
                lawfirm = partner_lawfirm_sudo.create({
                    'ifs_company_id': self.ifs_company_id.id,
                })

            factor_lawfirm_data.update({
                'lawfirm_id': lawfirm.id,
            })
            self.env['ifs.gar.partner.factor.lawfirm'].create(
                factor_lawfirm_data)
    ...
    def action_reject(self):
        if self.can_confirm:
            return {
                ...
                'type': 'ir.actions.act_window',
                'view_mode': 'form',
                'res_model': 'ifs.gar.review.lawfirm.reject.wizard',
                'target': 'new',
                'context': {
