...
class GuaranteeAccountsRecEntryDefinition(models.Model):
    _name = 'ifs.gar.entry.definition'
    _description = '进件流程参数设置'
...
class GuaranteeAccountsRecEntryMerchantConfig(models.Model):
    _name = 'ifs.gar.entry.merchant.config'
    _description = '采购方进件流程内容块配置'
    ...
    detail_ids = fields.One2many(
        'ifs.gar.entry.merchant.config.detail', 'config_id', string='进件流程参数设置明细')
    is_global = fields.Boolean('是否全局设置', compute='_compute_is_global')
    is_copy = fields.Boolean('是否可复制')
    ...
    def config_selector_supplier(self):
        return {
            'name': _('选择供应方'),
            'type': 'ir.actions.act_window',
            'view_mode': 'form',
            'res_model': 'ifs.gar.entry.config.supplier.selector.wizard',
            'res_id': False,
            'target': 'new',
            'context': {
    ...
    def copy_entry_config(self, factor_id, supplier_id):
        config = self.search([
        ...
        global_config = self.search([
            ('factor_id', '=', factor_id.id),
            ('supplier_id', '=', False)], limit=1)
        if global_config:
            detail_ids = []
            for detail in global_config.detail_ids:
                config_detail = self.env['ifs.gar.entry.merchant.config.detail'].browse(
                    detail.id).copy_data()[0]
                new_detail = self.env['ifs.gar.entry.merchant.config.detail'].create(
                    config_detail)
                detail_ids += [new_detail.id]
            new_config = self.create({
                'factor_id': factor_id.id,
                'supplier_id': supplier_id.id,
                'detail_ids': [fields.Command.set(detail_ids)]
            })
            return {
                'name': _(f'{new_config.display_name}'),
                'type': 'ir.actions.act_window',
                'view_mode': 'form',
                'res_model': 'ifs.gar.entry.merchant.config',
                'res_id': new_config.id,
                'target': 'current'
            }
        else:
            raise UserError(_('没有找到当前保理方的全局进件配置！'))


class GuaranteeAccountsRecEntryMerchantConfigDetail(models.Model):
    _name = 'ifs.gar.entry.merchant.config.detail'
    _description = '采购方进件流程参数设置明细'
    _order = 'sequence'

    sequence = fields.Integer('序号', default=10)
    config_id = fields.Many2one(
        'ifs.gar.entry.merchant.config', string='进件流程参数设置', required=True, index=True, ondelete='cascade')
    ...
    definition_id = fields.Many2one(
        'ifs.gar.entry.definition', string='属性配置', ondelete='restrict')
