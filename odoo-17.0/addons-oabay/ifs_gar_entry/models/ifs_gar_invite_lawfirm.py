...
class GuaranteeAccountsRecInviteLawfirm(models.Model):
    ...
    entry_ids = fields.One2many(
        'ifs.gar.entry.lawfirm', 'invite_id', string='进件流程s', copy=False)
    entry_id = fields.Many2one(
        'ifs.gar.entry.lawfirm', string='进件流程', compute='_compute_entry_id', copy=False)
    entry_date = fields.Datetime('进件时间', copy=False)
    
    lawfirm_id = fields.Many2one(
        'ifs.partner.lawfirm',
        string='律师事务所', index=True, ondelete='set null', help='邀请成功进件，成功加入后的律师事务所')
    lawfirm_code = fields.Char('律师事务所编号', related='lawfirm_id.seq_code')
        ...
        if not waiting_list or len(waiting_list) == 0:
            ...
            if waiting_list.state in ['draft', 'sended']:
                if waiting_list.entry_ids:
                    raise AccessDenied(_('当前律师事务所已经进件！'))
