...
class GuaranteeAccountsRecEntrySupplier(models.Model):
    _name = 'ifs.gar.entry.supplier'
    _inherit = ['ifs.gar.entry.mixin']
    _inherits = {'ifs.gar.invite.supplier': 'invite_id'}
    _description = '供应方进件流程'
    ...
    invite_id = fields.Many2one(
        'ifs.gar.invite.supplier', required=True, ondelete='restrict', index=True,
        string='邀请信息', help='此次进件对应的邀请信息', copy=True)
    last_entry_id = fields.Many2one(
        'ifs.gar.entry.supplier', string='上一次进件', copy=False)
    ...
    product_scope = fields.Text('提供的产品/服务')
    ...
    finance_name = fields.Char('财务联系人', copy=True)
    finance_phone = fields.Char('财务电话', copy=True)
    ...
    is_self_guarantee = fields.Boolean('是否自我担保', default=True)
