...
class GuaranteeAccountsRecInviteSupplier(models.Model):
    ...
    entry_ids = fields.One2many(
        'ifs.gar.entry.supplier', 'invite_id', string='进件流程s', copy=False)
    entry_id = fields.Many2one(
        'ifs.gar.entry.supplier', string='进件流程', compute='_compute_entry_id', copy=False)
    entry_date = fields.Datetime('进件时间', copy=False)

    supplier_id = fields.Many2one(
        'ifs.partner.supplier',
        string='供应方', index=True, ondelete='set null', help='邀请成功进件，成功加入后的供应方', copy=False)
        ...
        if not waiting_list or len(waiting_list) == 0:
            ...
            if waiting_list.state in ['draft', 'sended']:
                if waiting_list.entry_ids:
                    raise AccessDenied(_('当前供应方已经进件！'))
