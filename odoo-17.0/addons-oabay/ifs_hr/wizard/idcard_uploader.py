...
class IdcardUploader(models.TransientModel):
    _name = 'ifs.hr.idcard.uploader'
    _description = '身份证信息上传'
            ...
            try:
                ...
                raise RedirectWarning(
                    ...
                    action={
                        ...
                        'view_mode': 'form',
                        'views': [[self.env.ref('ifs_hr.ifs_hr_idcard_uploader_view_form').id, 'form']],
                        'target': 'new',
                        'context': {
    ...
    def action_confirm(self):
        idcard = self.env['hr.employee.idcard'].search([
            ('idcard_no', '=', self.idcard_no),
        ])
        if idcard.exists() and idcard.employee_ids.filtered(
                lambda e: e.id != self.employee_id.id).exists():
            raise RedirectWarning(
                message=_("身份证号码已存在，并已被其他用户绑定！"),
                button_text=_("重新上传"),
                action={
                    'name': _('身份证上传'),
                    'type': 'ir.actions.act_window',
        ...
        if idcard.exists():
            idcard.write(update_idcard)
        else:
            idcard = self.env['hr.employee.idcard'].create(update_idcard)
