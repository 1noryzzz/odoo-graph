...
def subscribe_handler(request, msg):
...
def unsubscribe_handler(request, msg):
    # openid = message.source
    # env = request.env()
    # rs = env['wx.corpuser'].sudo().search( [('userid', '=', openid)] )
    # if rs.exists():
...
def change_contact_handler(request, msg):
    request.env['wechat.work.user'].change_contact(msg)
    return ''
