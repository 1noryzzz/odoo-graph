"""Runtime probe script — executed INSIDE `odoo-bin shell`.

stdin script for `odoo-bin shell`. Expects `env` to be pre-populated.
Writes nodes.jsonl / edges.jsonl / edges_resolved.jsonl / summary.json to
$ODOO_GRAPH_OUT_DIR.

This is intentionally self-contained (no imports from odoo_graph) because
`odoo-bin shell` runs it as stdin with its own Python process and PYTHONPATH.
"""
...
OUT_DIR = os.environ["ODOO_GRAPH_OUT_DIR"]
...
registry = env.registry  # noqa: F821  — provided by odoo-bin shell
print(f"[odoo-graph] registry has {len(registry.models)} model classes", flush=True)
print(f"[odoo-graph] field_depends entries: {len(registry.field_depends)}", flush=True)
...
for mod_name in env.registry._init_modules:  # noqa: F821
    modules_seen.add(mod_name)

irmod = env["ir.module.module"].sudo()  # noqa: F821
...
for model_name, ModelCls in sorted(registry.models.items()):
    ...
    for fname, field in ModelCls._fields.items():
        ...
        if is_inherited:
            summary["fields_inherited_delegate"] += 1
        ...
        if field.compute:
            if isinstance(field.compute, str):
                compute_name = field.compute
            else:
                compute_name = getattr(field.compute, "__name__", None)
        ...
        if field.inverse:
            if isinstance(field.inverse, str):
                ...
                inverse_name = getattr(field.inverse, "__name__", None)
        ...
        if compute_name:
            emit_edge({
                ...
                "src": field_id,
                "dst": f"method::{model_name}.{compute_name}",
            })
        if inverse_name:
            emit_edge({
                "kind": "FIELD_INVERSE_BY",
                "src": field_id,
                "dst": f"method::{model_name}.{inverse_name}",
            })
...
# ---------- Methods + override inference via MRO ---------------------------
for model_name, ModelCls in sorted(registry.models.items()):
    seen_names = set()
...
print("[odoo-graph] wrote nodes:", os.path.join(OUT_DIR, "nodes.jsonl"), flush=True)
print("[odoo-graph] wrote edges:", os.path.join(OUT_DIR, "edges.jsonl"), flush=True)
print("[odoo-graph] summary:", json.dumps(summary), flush=True)
